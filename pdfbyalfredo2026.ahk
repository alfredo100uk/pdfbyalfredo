...
T["es"] := Map(
    "title", "PDF's ©by Alfredo 2026 v0.0.6",
...
; Botonera Lateral
BtnSubir    := MyGui.Add("Button", "x505 y35 w125 h28", T[idiomaActual]["subir"])
BtnBajar    := MyGui.Add("Button", "x505 y65 w125 h28", T[idiomaActual]["bajar"])
BtnAbrir    := MyGui.Add("Button", "x505 y95 w125 h28", T[idiomaActual]["abrirUno"])
BtnEliminar := MyGui.Add("Button", "x505 y125 w125 h28", T[idiomaActual]["eliminar"])
BtnLimpiar  := MyGui.Add("Button", "x505 y155 w125 h28", T[idiomaActual]["limpiar"])
...
ComprobarYDescargarDependencias() {
    ...
    if (!FileExist(sumatraPath)) {
        try {
            Download("https://www.sumatrapdfreader.org/dl/rel/3.5.2/SumatraPDF-3.5.2-64.exe", sumatraPath)
        } catch {
            try {
                Download("https://github.com/alfredo100uk/pdfbyalfredo/blob/main/SumatraPDF.exe", sumatraPath)
            } catch {
            }
        }
    }
...
AbrirPDFEnVentanaPropia(lvObj) {
    ...
    filePath := Trim(RegExReplace(lineaActual, "\s+[0-9\-,\s]+$", ""))
    
    if !FileExist(filePath)
        filePath := lineaActual
...
ActualizarTextosInterfaz() {
    global MyGui, LblDrop, ChkAlwaysOnTop, LV, BtnSubir, BtnBajar, BtnAbrir, BtnEliminar, BtnLimpiar
...
RotarPDFs(modoRotacion, sufijo, lvObj) {
    ...
    Loop total {
        ...
        filePath := Trim(RegExReplace(lineaCompleta, "\s+[0-9\-,\s]+$", ""))
        rangoPaginas := "1-end"
        if RegExMatch(lineaCompleta, ".*?(\d+-\d+)$", &m)
            rangoPaginas := m[1]
            
        if !FileExist(filePath)
            filePath := lineaCompleta
...
UnirPDFs(lvObj) {
    ...
    primerFilePath := Trim(RegExReplace(primerItemText, "\s+[0-9\-,\s]+$", ""))
    ...
    Loop total {
        ...
        filePath := Trim(RegExReplace(itemText, "\s+[0-9\-,\s]+$", ""))
        rango := "1-end"
